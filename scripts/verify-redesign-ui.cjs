...
(async () => {
 ...
 async function capture(name, theme, width) {
  ...
  const geometry = await page.evaluate(() => {
   const host=document.querySelector('#feature-route-host');
   const clipped=[...document.querySelectorAll('button,input,select,textarea')].filter(n=>{const r=n.getBoundingClientRect();return r.width&&r.height&&!n.closest('[hidden]')&&(r.left < -1||r.right>innerWidth+1)}).map(n=>n.id||n.textContent.slice(0,50));
 ...
 for(const theme of ['light','dark']) for(const width of [390,1280]) {
  ...
  await mount('notes',{noteId:'n1'});await capture('notes-editor',theme,width);
  for(const view of ['day','week','month']) {await mount('planner',{view,date:'2026-09-11'});await capture(`planner-${view}`,theme,width)}
  await mount('grades');await capture('grades-overview',theme,width);
  for(const tab of ['overview','assignments','graphs','what-if']) {await page.evaluate(tab=>reviewFixture.module.routeUpdate({courseId:'7',tab}),tab);if(tab==='graphs') await page.locator('[data-grades-role="generate-chart"]').click();await capture(`grades-${tab}`,theme,width)}
 }
 ...
 await page.route('https://canvas.emory.edu/**', async route => {
  ...
  if (url.pathname.startsWith('/api/')) return route.fulfill({json:url.pathname.includes('/users/self/profile')?{id:123,name:'Synthetic review student',short_name:'Review student'}:[]});
 ...
 for(const theme of ['light','dark'])for(const width of [390,1440]){
  ...
  for(const route of ['settings','notes','grades','planner']){
   ...
   const geometry=await frame.evaluate(()=>({route:APStudyCanvasWorkspace.route,featureWidth:document.querySelector('#feature-route-host')?.getBoundingClientRect().width,width:innerWidth,text:document.querySelector('#feature-route-host')?.innerText.slice(0,200)}));
   const hostState=await page.evaluate(()=>{const frame=[...document.querySelectorAll('*')].flatMap(n=>n.shadowRoot?[...n.shadowRoot.querySelectorAll('iframe')]:[])[0];const root=frame?.getRootNode();return{transformed:document.querySelector('#application')?.style.transform,previewVisible:!!root?.querySelector('.preview')?.getBoundingClientRect().width}});
